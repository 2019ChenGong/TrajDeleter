...
def main(args):
    ...
    for instance in unlearning_episodes:
        instance.rewards[:, ] = np.linspace(
            instance.rewards.min(), 
            instance.rewards.max(), 
            len(instance.rewards[:, ])
        )
    
    #     #fine-tuning
        
    train_episodes.extend(unlearning_episodes)
    
    log = "./Mujoco_noise_reward/" + str(args.dataset) + '-' + str(args.number_of_unlearning)  + '-' + str(args.unlearning_rate-0.1) + '/'
    
    algorithm.fit(train_episodes,
            eval_episodes=test_episodes_,
            n_steps=args.number_of_unlearning,
            n_steps_per_epoch=args.number_of_unlearning,
            logdir=log,
            scorers={
    ...
    # file_name = log + 'Running_Time_' + str(args.dataset) + '_' + str(args.algo) + '_' + str(args.unlearning_rate) + '_' + str(args.number_of_unlearning) + '.csv'
