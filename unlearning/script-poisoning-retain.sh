...
declare -A datasets
datasets["hopper-medium-v0"]="hopper"
datasets["halfcheetah-medium-v0"]="half"
datasets["walker2d-medium-v0"]="walk"
...
for dataset_key in "${!datasets[@]}"; do
    ...
    for algo_key in "${!algos[@]}"; do
        ...
        for seed in 0 1 2; do
            ...
            command="CUDA_VISIBLE_DEVICES=$gpu_id python ./poisoning_retrain.py --seed=$seed --dataset='$dataset_key' --model='$model_path' --algo='$algo_key' &"
            echo $command
            eval $command
            sleep 1
